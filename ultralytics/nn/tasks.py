...
from ultralytics.nn.modules.layers.AttF import (
    ACF,
    LightAttentionFusion,
    C2f_FCA,
    MER,
)
from ultralytics.nn.modules.layers.BiFPN import(
    AMA, DWSConv
) 
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [c2, 2]  # [in_channels, reduction]

 

        elif m is ACF:
            # ACF expects: [input_channels, output_channels, num_heads, dropout]
            c2 = args[0]  # output channels
            if c2 != nc:  # if not equal to number of classes
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            
            # compute input channels (from concat of multiple sources)
            c1 = sum(ch[x] for x in f) if isinstance(f, list) else ch[f]
            
            # set default parameters
            num_heads = args[1] if len(args) > 1 else 8
            dropout = args[2] if len(args) > 2 else 0.1
            
            args = [c1, c2, num_heads, dropout]

        elif m is LightAttentionFusion:
            # LightAttentionFusion expects: [input_channels, output_channels]
            c2 = args[0]  # output channels
            if c2 != nc:  # if not equal to number of classes
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            ...
            # AMA receives multiple inputs [P2, P3, P4, P5]
            if isinstance(f, list):
                # f is a list, e.g. [26, 27, 28, 29]
                c1 = [ch[x] for x in f]  # get channel count for each input
                c2 = args[0]  # output channel count
                args = [c1, c2, *args[1:]]  # [input channel list, output channel count, other args]
                # AMA outputs four feature layers, each with the same channel count
                c2 = [c2, c2, c2, c2]  # [P2_out, P3_out, P4_out, P5_out]
            else:
                # single input case (should not happen with AMA)
                c1 = ch[f]
                c2 = args[0]
                args = [c1, c2, *args[1:]]
                c2 = [c2, c2, c2, c2]  # maintain consistency

        elif m is DWSConv:
            # DWSConv: depthwise separable convolution
            c1 = ch[f]
            c2 = args[0] if args[0] != -1 else c1
            args = [c1, c2, *args[1:]]
            ...
            # Index layer selects tensor at specific index from list input
            index = args[0]
            input_channels = ch[f]
            if isinstance(input_channels, list):
                # if input is a multi-channel list, select channels at target index
                c2 = input_channels[index]
            else:
                # if input is single channel count, keep unchanged
                c2 = input_channels
