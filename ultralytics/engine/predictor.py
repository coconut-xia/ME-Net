...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        """
        self.dvs_results = None  # for DVS results
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                with profilers[2]:
                    if self.args.ch >= 4:  # dual-modality (RGB+DVS)
                        self.results, self.dvs_results = self.postprocess(preds, im, im0s)
                    else:
                ...
                for i in range(n):
                    ...
                    if self.dvs_results is not None:  # for DVS results
                        if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                            s[i] += self.write_results(
                                i, Path(paths[i][:-4] + "_dvs" + paths[i][-4:]), im[:, 3:], s, self.dvs_results
                            )
